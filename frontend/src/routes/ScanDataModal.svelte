    ...
    try {
      ...
      if (results && results.length > 0) {
        ...
        if (match) {
          ...
          errorMsg = $t('noScanDataFound');
        }
      } else {
        errorMsg = $t('noScanResultsAvailable');
      }
    } catch (err) {
      errorMsg = $t('failedToLoadData', { err: err.message || err });
    } finally {
      loading = false;
    }
  }
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-3xl max-h-[85vh] flex flex-col shadow-2xl relative text-slate-200">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/40 rounded-t-2xl">
        <div>
          <h3 class="text-md font-bold bg-gradient-to-r from-sky-400 to-indigo-400 bg-clip-text text-transparent flex items-center gap-2 font-sans">
            ...
            {$t('scanDataDetailsTitle')}
          </h3>
          <p class="text-xxs text-slate-400 mt-0.5 font-sans">
            {#if node}
              {$t('targetDevice')} <span class="text-slate-200 font-semibold">{node.label || node.ip || node.id}</span>
      ...
      <div class="flex-grow overflow-y-auto p-6 space-y-6">
        ...
        <div class="bg-slate-950/50 border border-slate-800 rounded-xl p-4 text-xs text-slate-300 space-y-2 font-sans">
          <p class="font-semibold text-sky-400">{$t('referenceInfoTitle')}</p>
        ...
        {#if loading}
          <div class="flex flex-col items-center justify-center py-20 text-slate-500 gap-3 font-sans">
            ...
            <p class="text-xs">{$t('loadingScanData')}</p>
          </div>
        {:else if errorMsg}
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="bg-slate-950/30 border border-slate-850 rounded-xl p-4 space-y-3">
              ...
              <div class="text-xs space-y-2 font-sans text-slate-300">
                <div class="flex justify-between"><span class="text-slate-500">{$t('ipAddress')}:</span> <span class="font-mono">{scanData.ip || '—'}</span></div>
                <div class="flex justify-between"><span class="text-slate-500">{$t('macAddress')}:</span> <span class="font-mono">{scanData.mac || '—'}</span></div>
                <div class="flex justify-between"><span class="text-slate-500">{$t('ouiVendor')}:</span> <span class="text-slate-200">{scanData.vendor || '—'}</span></div>
                <div class="flex justify-between"><span class="text-slate-500">{$t('addNodeSysName')}:</span> <span class="text-sky-400 font-semibold">{scanData.sysName || '—'}</span></div>
                <div class="flex flex-col gap-1 mt-1">
                  <span class="text-slate-500">{$t('addNodeSysDesc')}:</span>
            ...
            <div class="bg-slate-950/30 border border-slate-850 rounded-xl p-4 space-y-3">
              ...
              <div class="text-xs space-y-3 font-sans text-slate-300">
                <div>
                  <span class="text-slate-500 block mb-1">{$t('openPortsTcp')}</span>
                ...
                <div>
                  <span class="text-slate-500 block mb-1">{$t('lldpNeighbors')}</span>
                      ...
                      {#each scanData.lldpNeighbors as nb}
                        <div class="bg-slate-950/40 border border-slate-800/80 rounded p-2 text-xxs text-slate-400">
                          <div class="flex justify-between font-semibold text-slate-300">
                            <span>{$t('addNodeSysName')}: {nb.sysName || '—'}</span>
                  ...
                  {:else}
                    <span class="text-slate-600 italic">{$t('noLldpNeighbors')}</span>
                  {/if}
                </div>
              </div>
            </div>
